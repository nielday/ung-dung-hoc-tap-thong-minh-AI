...
export default function AIStudyMode({ lectureData }: AIStudyModeProps) {
  ...
  const saveProgress = async () => {
    if (!lectureData || !lectureData.id) return
  ...
  const loadProgress = async () => {
    if (!lectureData || !lectureData.id) return false
    
    try {
      const userId = localStorage.getItem('user') ? JSON.parse(localStorage.getItem('user')!).id : null
      if (!userId) return false

      const response = await fetch(`/api/study-progress?userId=${userId}&lectureId=${lectureData.id}`)
      const data = await response.json()
      ...
      if (data.success && data.progress) {
        ...
        if (progress.quizProgress) {
          setCurrentQuestionIndex(progress.quizProgress.currentQuestionIndex || 0)
          setQuizScore(progress.quizProgress.quizScore || 0)
          setIsQuizComplete(progress.quizProgress.isComplete || false)
        }
        
        // Restore review progress
        if (progress.reviewProgress) {
          setCurrentReviewIndex(progress.reviewProgress.currentIndex || 0)
          setReviewNotes(progress.reviewProgress.notes || {})
          setIsReviewComplete(progress.reviewProgress.isComplete || false)
        }
        
        // Restore practice progress
        if (progress.practiceProgress) {
          setCurrentExerciseIndex(progress.practiceProgress.currentIndex || 0)
          setExerciseAnswers(progress.practiceProgress.answers || {})
          setIsPracticeComplete(progress.practiceProgress.isComplete || false)
        }
        
        // Restore session data
        if (progress.sessionData && progress.sessionData.startTime) {
  ...
  const clearProgress = async () => {
    ...
    try {
      ...
      if (response.ok) {
        console.log('Progress cleared from database successfully')
        // Reset all states
  ...
  useEffect(() => {
    const initializeStudyMode = async () => {
      if (lectureData && lectureData.content) {
        ...
        // Try to load existing quiz from database first
        const quizLoaded = await loadExistingQuiz()
        if (!quizLoaded) {
          // If no existing quiz found, generate new quiz
  ...
  useEffect(() => {
    ...
    if (lectureData && lectureData.id && (currentQuestionIndex > 0 || quizScore > 0 || isQuizComplete)) {
      saveProgress()
    }
  ...
  useEffect(() => {
    ...
    if (lectureData && lectureData.id && (currentReviewIndex > 0 || Object.keys(reviewNotes).length > 0 || isReviewComplete)) {
      saveProgress()
    }
  ...
  useEffect(() => {
    ...
    if (lectureData && lectureData.id && (currentExerciseIndex > 0 || Object.keys(exerciseAnswers).length > 0 || isPracticeComplete)) {
      saveProgress()
    }
  ...
  const loadExistingQuiz = async () => {
    ...
    try {
      console.log('🔄 Loading existing quiz from database for lecture:', lectureData.id)
      const response = await fetch(`/api/quiz?lectureId=${lectureData.id}`)
      const data = await response.json()
      
      if (data.success && data.quizzes && data.quizzes.length > 0) {
        // Use the first quiz found
        const quiz = data.quizzes[0]
        console.log('✅ Found existing quiz:', quiz.title, 'with', quiz.questions?.length || 0, 'questions')
      ...
      console.log('ℹ️ No existing quiz found in database')
      return false
    } catch (error) {
        ...
        <div className="mb-6">
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between mb-4 space-y-2 sm:space-y-0">
            ...
            <div className="flex flex-col sm:flex-row sm:items-center gap-2 sm:gap-2">
              <span className={`badge-${currentQuestion.difficulty === 'easy' ? 'success' : currentQuestion.difficulty === 'medium' ? 'warning' : 'danger'} text-xs sm:text-sm`}>
          ...
          <div className="card p-4 sm:p-6 lg:p-8 text-center mb-6">
            <h3 className="text-lg sm:text-xl font-semibold mb-3 sm:mb-4">{t('study.quiz.question')}:</h3>
    ...
    return (
      ...
      >
        ...
        <div className="card p-4 sm:p-6 lg:p-8 mb-6">
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between mb-4 space-y-2 sm:space-y-0">
            <h3 className="text-lg sm:text-xl font-semibold">{currentItem.title}</h3>
            <span className={`badge-${currentItem.importance === 'high' ? 'danger' : currentItem.importance === 'medium' ? 'warning' : 'success'} text-xs sm:text-sm`}>
          ...
          <div className="mb-4 sm:mb-6">
            <p className="text-base sm:text-lg text-gray-700 leading-relaxed">{currentItem.content}</p>
          </div>
  ...
  if (activeMode === 'practice') {
    ...
    return (
      ...
      >
        ...
        <div className="card p-4 sm:p-6 lg:p-8 mb-6">
          <h3 className="text-lg sm:text-xl font-semibold mb-3 sm:mb-4">{currentExercise.title}</h3>
          <p className="text-sm sm:text-base text-gray-600 mb-4 sm:mb-6">{currentExercise.description}</p>
    ...
    return (
      ...
      >
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between mb-6 space-y-3 sm:space-y-0">
          ...
          <div className="flex flex-col sm:flex-row gap-2">
            <button onClick={() => setActiveMode('quiz')} className="btn-primary text-xs sm:text-sm">
              <Target className="w-4 h-4 mr-2" />
              {t('study.analytics.startQuiz')}
            </button>
            <button onClick={() => setActiveMode('review')} className="btn-accent text-xs sm:text-sm">
              <BookOpen className="w-4 h-4 mr-2" />
              {t('study.analytics.startReview')}
            </button>
            <button onClick={() => setActiveMode('practice')} className="btn-warning text-xs sm:text-sm">
              <Zap className="w-4 h-4 mr-2" />
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6 mb-6 sm:mb-8">
          <div className="card p-4 sm:p-6 text-center">
            <div className="text-2xl sm:text-3xl font-bold text-blue-600 mb-2">{totalSessions}</div>
            <div className="text-xs sm:text-sm text-gray-600">{t('study.analytics.studySessions')}</div>
          </div>
          <div className="card p-4 sm:p-6 text-center">
            <div className="text-2xl sm:text-3xl font-bold text-green-600 mb-2">{totalQuestions}</div>
            <div className="text-xs sm:text-sm text-gray-600">{t('study.analytics.totalQuestions')}</div>
          </div>
          <div className="card p-4 sm:p-6 text-center">
            <div className="text-2xl sm:text-3xl font-bold text-purple-600 mb-2">{averageScore}%</div>
            <div className="text-xs sm:text-sm text-gray-600">{t('study.analytics.averageScore')}</div>
          </div>
        </div>
    ...
    >
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-6">
        ...
        >
          ...
          <h3 className="text-base sm:text-lg font-semibold mb-2">{t('study.smartQuiz')}</h3>
          <p className="text-sm sm:text-base text-gray-600">{t('study.smartQuizDesc')}</p>
        </motion.button>
        ...
        <motion.button
          ...
          whileHover={{ y: -5 }}
        >
          <BookOpen className="w-10 h-10 sm:w-12 sm:h-12 text-green-600 mx-auto mb-3 sm:mb-4" />
          <h3 className="text-base sm:text-lg font-semibold mb-2">{t('study.smartReview')}</h3>
          <p className="text-sm sm:text-base text-gray-600">{t('study.smartReviewDesc')}</p>
        </motion.button>
        ...
        <motion.button
          ...
          whileHover={{ y: -5 }}
        >
          <Zap className="w-10 h-10 sm:w-12 sm:h-12 text-yellow-600 mx-auto mb-3 sm:mb-4" />
          <h3 className="text-base sm:text-lg font-semibold mb-2">{t('study.practice')}</h3>
          <p className="text-sm sm:text-base text-gray-600">{t('study.practiceDesc')}</p>
        </motion.button>
        ...
        >
          ...
          <h3 className="text-base sm:text-lg font-semibold mb-2">{t('study.studyAnalytics')}</h3>
          <p className="text-sm sm:text-base text-gray-600">{t('study.studyAnalyticsDesc')}</p>
        </motion.button>
      </div>
    </motion.div>
  )
