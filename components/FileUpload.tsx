  ...
  const processFile = async (fileData: FileData) => {
    try {
      ...
      if (processedFiles.has(fileData.id)) {
        console.log('⚠️ File already processed, skipping:', fileData.id)
        return
      }
      
      // Mark file as being processed
      setProcessedFiles(prev => new Set(prev).add(fileData.id))
      console.log('🔄 Processing file:', fileData.id, fileData.file.name)
      ...
      try {
        ...
        if (user.id) {
          ...
          if (lectureResponse.ok) {
            ...
            console.log('📤 Calling onFileProcessed with complete lecture data')
            ...
            console.log('📤 Calling onFileProcessed with fallback data (no DB save)')
            onFileProcessed(processedData)
          }
        } else {
          // No user ID, pass processed data with timestamp ID
          console.log('📤 Calling onFileProcessed with fallback data (no user ID)')
          onFileProcessed(processedData)
        }
      } catch (error) {
        console.error('Failed to save lecture to database:', error)
        // Fallback: pass processed data with timestamp ID
        console.log('📤 Calling onFileProcessed with fallback data (error)')
        onFileProcessed(processedData)
      }
